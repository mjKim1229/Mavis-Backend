...
class InquiryControllerTest extends ControllerTestSupport {
    ...
    class 상품_문의_목록_조회 {
        ...
        @Test
        void 답변_포함_조회() throws Exception {
        ...
        @Test
        void 비공개_문의_내용_노출_안됨() throws Exception {
        ...
        @Test
        void 미답변만_필터링() throws Exception {
            ...
            mockMvc.perform(get("/v1/api/inquiry/product/{id}", savedProduct.getId())
                    ...
                    .andExpect(jsonPath("$.data.content[0].isPrivate").value(false))
                    .andExpect(jsonPath("$.data.content[0].answerStatus").value("UNANSWERED"))
                    .andExpect(jsonPath("$.data.content[0].inquiry.id").value(unanswered.getId()))
    ...
    class 사용자_문의_목록_조회 {
        ...
        @Test
        void 다른_유저_문의_미포함() throws Exception {
            User otherUser = userRepository.save(User.builder()
                    .name("다른유저")
                    .email("other@test.com")
                    .build());
            Inquiry myInquiry = inquiryRepository.save(Inquiry.builder()
                    .question("내 질문")
                    .isPrivate(false)
                    .product(savedProduct)
                    .user(savedUser)
            ...
            mockMvc.perform(get("/v1/api/inquiry/user")
                    ...
                    .andExpect(status().isOk())
                    .andExpect(jsonPath("$.data.totalElements").value(1))
                    .andExpect(jsonPath("$.data.content[0].id").value(myInquiry.getId()))
                    .andExpect(jsonPath("$.data.content[0].question").value("내 질문"));
        }
